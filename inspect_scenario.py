...
async def run_scenario():
    artifact_dir = r"C:\Users\milch\.gemini\antigravity\brain\d585f72a-9bd9-4708-9c4b-b6a6c99c6df2"
    ...
    try:
        ...
        async with websockets.connect(ws_url) as ws:
            ...
            # Expand subtasks if any are collapsed, or verify subtask list expand buttons
            # We want to check for layout misalignment of subtasks in list view
            print("Expanding subtasks in List View to check alignment...")
            expand_subtasks_js = """
            (() => {
                const expandBtns = Array.from(document.querySelectorAll('.subtask-toggle-btn'));
                expandBtns.forEach(btn => btn.click());
                return expandBtns.length;
            })()
            """
            expanded_count = await eval_js(ws, expand_subtasks_js)
            print(f"Expanded {expanded_count} subtask toggle buttons.")
            await asyncio.sleep(1)
            
            await capture_and_save_screenshot(ws, os.path.join(artifact_dir, "2_list_view.png"))
            
            # Measure column alignment of headers, main tasks, and subtasks
            alignment_js = """
            (() => {
                const result = {};
                
                // Get header column positions
                const headerRow = document.querySelector('.list-view-row.header');
                if (headerRow) {
                    result.headers = Array.from(headerRow.querySelectorAll('.col')).map(c => {
                        const r = c.getBoundingClientRect();
                        return { class: c.className, left: r.left, width: r.width, text: c.textContent.trim() };
                    });
                }
                
                // Get main tasks column positions
                const mainTasks = Array.from(document.querySelectorAll('.list-view-row.main-task'));
                result.mainTasks = mainTasks.map(t => {
                    const titleText = t.querySelector('.task-title-text')?.textContent.trim();
                    const cols = Array.from(t.querySelectorAll('.col')).map(c => {
                        const r = c.getBoundingClientRect();
                        return { class: c.className, left: r.left, width: r.width };
                    });
                    return { title: titleText, cols: cols };
                });
                
                // Get subtasks column positions
                const subTasks = Array.from(document.querySelectorAll('.list-view-row.sub-task'));
                result.subTasks = subTasks.map(t => {
                    const titleText = t.querySelector('.task-title-text')?.textContent.trim();
                    const cols = Array.from(t.querySelectorAll('.col')).map(c => {
                        const r = c.getBoundingClientRect();
                        return { class: c.className, left: r.left, width: r.width };
            ...
            """
            column_alignment = await eval_js(ws, alignment_js)
            print("Column alignment data extracted.")
            ...
            await asyncio.sleep(1)
            await capture_and_save_screenshot(ws, os.path.join(artifact_dir, "3_task_modal_open.png"))
            
            # Check for column elements in modal and measure sizes/positions
            modal_checks_js = """
            (() => {
                ...
                results.overlaps = [];
                const cols = [
                    { name: 'Tree Sidebar', el: treeCol },
                    { name: 'Details Pane', el: detailsCol },
                    { name: 'Comments Pane', el: commentsCol }
                ].filter(c => c.el);
                
                for (let i = 0; i < cols.length; i++) {
                    for (let j = i + 1; j < cols.length; j++) {
                        const r1 = cols[i].el.getBoundingClientRect();
                        const r2 = cols[j].el.getBoundingClientRect();
                        const overlap = !(r1.right <= r2.left || r1.left >= r2.right || r1.bottom <= r2.top || r1.top >= r2.bottom);
                        if (overlap) {
                            results.overlaps.push(`${cols[i].name} overlaps ${cols[j].name}`);
                        }
                    }
                }
                return results;
            ...
            # Try to upload screenshot attachment
            ...
            await capture_and_save_screenshot(ws, os.path.join(artifact_dir, "4_screenshot_uploaded.png"))
            ...
            await asyncio.sleep(1)
            await capture_and_save_screenshot(ws, os.path.join(artifact_dir, "5_lightbox_open.png"))
    ...
    report_path = os.path.join(artifact_dir, "inspection_results.json")
